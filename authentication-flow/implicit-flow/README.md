...
```bash
  ...
  # Após iniciar, acesse o aplicativo no navegador:
  http://localhost:3000
```
...
- npm run <script> -w <workspace>: executa scripts em um workspace específico.
- npm outdated -ws: mostra desatualizados em todos os workspaces.
- npm add <package> -w <workspace>: instala pacote em um workspace específico.
